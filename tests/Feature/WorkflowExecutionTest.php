...
class WorkflowExecutionTest extends TestCase
{
    ...
    public function test_partial_po_receipt_posts_lot_stock_and_keeps_po_open(): void
    {
        ...
        $this->actingAs($user)->post("/admin/procurement/{$poId}/receipts", ['received_date' => '2026-08-26', 'warehouse_id' => $warehouseId, 'location_id' => $locationId, 'items' => [['po_item_id' => $poItemId, 'quantity' => 4, 'lot_roll_no' => 'ROLL-1', 'material_color' => 'Green', 'material_size' => 'M']]])
    ...
    private function createWorkflowSchema(): void
    {
        ...
        Schema::create('po_receipt_items', function (Blueprint $table): void { $table->id(); $table->unsignedBigInteger('po_receipt_id'); $table->unsignedBigInteger('po_item_id'); $table->string('material_code'); $table->string('material_color')->nullable(); $table->string('material_size')->nullable(); $table->decimal('quantity_received', 14, 4); $table->string('batch_no')->nullable(); $table->unsignedBigInteger('warehouse_id'); $table->unsignedBigInteger('location_id')->nullable(); $table->unsignedBigInteger('material_id')->nullable(); $table->timestamps(); });

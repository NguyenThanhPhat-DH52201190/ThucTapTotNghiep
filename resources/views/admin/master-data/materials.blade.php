...
<div class="modal fade" id="taxonomyModal"><div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Manage categories</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="row g-4">
 <div class="col-md-6"><h6>Categories</h6><form method="POST" action="{{ route('admin.master-data.material-categories.store') }}" class="input-group mb-3">@csrf<input name="name" class="form-control" placeholder="New category" required><button class="btn btn-primary">Add</button></form>
 <div id="categoryList">@forelse($categories as $category)<div class="border rounded p-2 mb-2 d-flex gap-2 taxonomy-category" data-name="{{ Str::lower($category->name) }}"><form method="POST" action="{{ route('admin.master-data.material-categories.update', $category->id) }}" class="d-flex gap-2 flex-grow-1">@csrf @method('PATCH')<input name="name" value="{{ $category->name }}" class="form-control form-control-sm" required><button class="btn btn-sm btn-outline-primary"><i class="bi bi-check-lg"></i></button></form><form method="POST" action="{{ route('admin.master-data.material-categories.destroy', $category->id) }}" onsubmit="return confirm('Delete this category?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div>@empty<p class="text-muted">No categories yet.</p>@endforelse</div></div>
 <div class="col-md-6"><h6>Subcategories</h6><form method="POST" action="{{ route('admin.master-data.material-subcategories.store') }}" class="input-group mb-3">@csrf<select name="category_id" class="form-select" required><option value="">Category</option>@foreach($categories as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</select><input name="name" class="form-control" placeholder="New subcategory" required><button class="btn btn-primary">Add</button></form>
 <div class="mb-3"><select id="subcategoryCategoryFilter" class="form-select form-select-sm"><option value="">All categories</option>@foreach($categories as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</select></div>
 <div id="subcategoryList">@forelse($subcategories as $subcategory)<div class="border rounded p-2 mb-2 d-flex gap-2 align-items-start taxonomy-subcategory" data-category="{{ $subcategory->category_id }}" data-name="{{ Str::lower($subcategory->name) }}"><form method="POST" action="{{ route('admin.master-data.material-subcategories.update', $subcategory->id) }}" class="d-flex gap-2 flex-grow-1 taxonomy-subcategory-update">@csrf @method('PATCH')<select name="category_id" class="form-select form-select-sm" required>@foreach($categories as $category)<option value="{{ $category->id }}" @selected($subcategory->category_id == $category->id)>{{ $category->name }}</option>@endforeach</select><input name="name" value="{{ $subcategory->name }}" class="form-control form-control-sm" required><button class="btn btn-sm btn-outline-primary flex-shrink-0"><i class="bi bi-check-lg"></i></button></form><form method="POST" action="{{ route('admin.master-data.material-subcategories.destroy', $subcategory->id) }}" class="flex-shrink-0" onsubmit="return confirm('Delete this subcategory?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div>@empty<p class="text-muted">No subcategories yet.</p>@endforelse</div></div>
</div></div></div></div></div>

<div class="modal fade" id="mappingModal"><div class="modal-dialog"><form method="POST" action="{{ route('admin.master-data.material-vendors.store') }}" class="modal-content">@csrf<div class="modal-header"><h5 class="modal-title">Map material to supplier</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="row g-3">
...
function taxonomyCategoryName(item){const select=item.querySelector('select[name="category_id"]'),option=select?.options[select.selectedIndex];return (option?.textContent||'').trim().toLocaleLowerCase()}
function applyCategoryFilter(){document.querySelectorAll('.taxonomy-category').forEach(item=>item.hidden=false);sortTaxonomy('categoryList','.taxonomy-category')}
function applySubcategoryFilter(){const category=document.getElementById('subcategoryCategoryFilter')?.value||'',list=document.getElementById('subcategoryList');document.querySelectorAll('.taxonomy-subcategory').forEach(item=>{const currentCategory=item.querySelector('select[name="category_id"]')?.value||item.dataset.category;item.hidden=!!category&&currentCategory!==category});[...list.querySelectorAll('.taxonomy-subcategory')].sort((a,b)=>taxonomyCategoryName(a).localeCompare(taxonomyCategoryName(b),undefined,{sensitivity:'base'})||taxonomyName(a).localeCompare(taxonomyName(b),undefined,{sensitivity:'base'})).forEach(item=>list.appendChild(item))}
document.getElementById('subcategoryCategoryFilter')?.addEventListener('change',applySubcategoryFilter);
document.getElementById('subcategoryList')?.addEventListener('change',event=>{if(event.target.matches('select[name="category_id"]'))applySubcategoryFilter()});
document.getElementById('taxonomyModal')?.addEventListener('shown.bs.modal',()=>{applyCategoryFilter();applySubcategoryFilter()});
</script>
@endpush

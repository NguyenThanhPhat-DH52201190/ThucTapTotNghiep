...
<div class="container-fluid px-0">
    ...
    <div class="card shadow-sm border-0"><div class="table-responsive"><table class="table table-bordered table-hover align-middle mb-0">
        <thead class="table-light"><tr><th>CS</th><th>Style</th><th>Style Name</th><th>Customer</th><th>Color</th><th class="text-end">Qty</th><th>BOM</th><th>Status</th><th>Action</th></tr></thead>
        <tbody>@forelse($orders as $order)<tr>
            <td class="fw-bold">{{ $order->CS }}</td><td>{{ $order->SNo }}</td><td>{{ $order->Sname }}</td><td>{{ $order->Customer }}</td><td>{{ $order->Color }}</td><td class="text-end fw-semibold">{{ number_format($order->Qty, 0) }}</td>
            <td>{{ $order->bom_style }} / {{ $order->bom_version }}</td><td><span class="badge bg-secondary">{{ ucfirst(str_replace('_', ' ', $order->status)) }}</span></td>
        ...
        </tr>@empty<tr><td colspan="9" class="text-center text-muted py-4">No OCS with a BOM found.</td></tr>@endforelse</tbody>

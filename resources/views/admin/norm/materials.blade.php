...
<div class="container-fluid px-0">
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <div><h5 class="mb-1 fw-bold"><i class="bi bi-rulers me-2"></i>NORM Materials — {{ $order->CS }}</h5><small class="text-muted">{{ $order->SNo }} — {{ $order->Sname }} · Qty {{ number_format($order->Qty, 0) }}</small></div>
    ...
    <div class="card shadow-sm border-0"><div class="table-responsive"><table class="table table-bordered table-hover align-middle mb-0">
        ...
        <tbody>@forelse($rows as $row)<tr>
            <td class="fw-semibold">{{ $row->CS }}</td><td>{{ $row->SNo }}</td><td><code>{{ $row->material_code }}</code></td><td>{{ $row->material_name }}</td><td><span class="badge bg-info">{{ ucfirst($row->material_type) }}</span></td>

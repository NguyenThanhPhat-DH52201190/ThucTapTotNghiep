<div class="defect-row border rounded p-3 mb-3">
    <div class="row g-3">
        <div class="col-md-6"><label class="form-label">Material *</label><select data-field="bom_item_id" name="items[{{ $index }}][bom_item_id]" class="form-select" required>
            <option value="">Select material</option>
            @foreach($materials as $material)<option value="{{ $material->bom_item_id }}" @selected((string) ($item['bom_item_id'] ?? '') === (string) $material->bom_item_id)>{{ $material->material_code }} — {{ $material->material_name }} / {{ $material->material_color }} / {{ $material->material_size }} ({{ $material->unit }})</option>@endforeach
        </select></div>

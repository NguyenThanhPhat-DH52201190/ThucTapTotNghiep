...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('cost_analyses', function (Blueprint $table) {
            ...
            $table->decimal('standard_fabric_cost', 18, 4)->default(0);   // Chi phí vải chuẩn
            $table->decimal('standard_trim_cost', 18, 4)->default(0);     // Chi phí phụ liệu chuẩn
            $table->decimal('standard_labor_cost', 18, 4)->default(0);    // Chi phí nhân công chuẩn
            $table->decimal('standard_cmt_cost', 18, 4)->default(0);      // Chi phí CMT chuẩn
            $table->decimal('standard_overhead_cost', 18, 4)->default(0); // Chi phí sản xuất chung
            $table->decimal('standard_total_cost', 18, 4)->default(0);    // Tổng chi phí chuẩn

            // Actual costs (từ sản xuất thực tế)
            $table->decimal('actual_fabric_cost', 18, 4)->default(0);
            $table->decimal('actual_trim_cost', 18, 4)->default(0);
            $table->decimal('actual_labor_cost', 18, 4)->default(0);
            $table->decimal('actual_cmt_cost', 18, 4)->default(0);
            $table->decimal('actual_overhead_cost', 18, 4)->default(0);
            $table->decimal('actual_total_cost', 18, 4)->default(0);

            // Revenue & Profit
            $table->decimal('revenue_per_unit', 18, 4)->default(0);       // Giá bán/đơn vị
            ...
            // Variance
            $table->decimal('cost_variance', 18, 4)->default(0);          // Chênh lệch (Actual - Standard)
